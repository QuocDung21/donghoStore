<div class="table-responsive">
    <table class="table table-borderless table-theme" style="width:100%;">
        <thead>
        <tr>
            <th class="text-left" style="width:7%">{{ __('Order#') }}</th>
            ...
            <th class="text-center " style="width:5%">{{ __('Payment Method') }}</th>
        ...
        @if (count($datalist)>0)
            ...
            @foreach($datalist as $row)
                ...
                @endphp
                <tr>
                    <td class="text-left"><a href="{{ route('seller.order', [$row->id]) }}">{{ $row->order_no }}</a>
                    </td>
                    ...
                    <td class="text-center w-25"><span
                                class="status_btn pstatus_{{ $row->payment_status_id }}">{{ __($row->pstatus_name) }}</span>
                    </td>
                    <td class="text-center w-25"><span
                                class=" status_btn ostatus_{{ $row->order_status_id }}">{{ __($row->ostatus_name) }}</span>
                    </td>
                        ...
                        <div class="btn-group action-group">
                            ...
                            <div class="dropdown-menu dropdown-menu-right">
                                <a class="dropdown-item"
                                   href="{{ route('seller.order', [$row->id]) }}">{{ __('View') }}</a>
                                <a class="dropdown-item"
                                   href="{{ route('frontend.order-invoice', [$row->id, $row->order_no]) }}">{{ __('Invoice') }}</a>
