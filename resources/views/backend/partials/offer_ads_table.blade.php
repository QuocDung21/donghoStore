<div class="table-responsive">
    <table class="table table-borderless table-theme" style="width:100%;">
        ...
        <tr>
            ...
            <th class="text-left d-none" style="width:15%">{{ __('Offer & Ads Position') }}</th>
        ...
        @if (count($datalist)>0)
            @foreach($datalist as $row)
                <tr>
                    ...
                    <td class="text-left d-none">{{ $row->offer_ad_type }}</td>
                    <td class="text-left"><a target="_blank" href="{{ $row->url }}">{{ $row->url }}</a></td>

                    @if ($row->image != '')
                        <td class="text-center">
                            <div class="table_col_image"><img src="{{ asset('public') }}/media/{{ $row->image }}"/>
                            </div>
                        </td>
                    @else
                        <td class="text-center">
