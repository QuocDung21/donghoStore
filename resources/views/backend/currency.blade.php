								...
								<div class="col-md-6">
									<div class="form-group">
										...
										<select name="currency_position" id="currency_position" class="chosen-select form-control">
											<option {{ 'left' == $datalist['currency_position'] ? "selected=selected" : '' }} value="left">
												{{__('Left')}}</option>
											<option {{ 'right' == $datalist['currency_position'] ? "selected=selected" : '' }} value="right">{{ __('Right') }}</option>
										</select>
									</div>
								</div>
							</div>
